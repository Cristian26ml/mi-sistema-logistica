...
class Product(models.Model):
    ...
    # controlado por reglas/servicios
    ...
    def save(self, *args, **kwargs):
        ...
        if creando or not self.codigo_interno:
            if not self.codigo_interno:
                updates["codigo_interno"] = f"INT-{self.id:06d}"

        if creando or not self.codigo_barra:
            if not self.codigo_barra:
                # Base de 12 dígitos para EAN-13
                # Ejemplo: prefijo empresa + id relleno
                base12 = f"200{self.id:09d}"  # 3 + 9 = 12
                updates["codigo_barra"] = generar_ean13_desde_base(base12)
